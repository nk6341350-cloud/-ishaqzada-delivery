...
/* Mobile compatibility helpers — no business logic changes */
...
function safeRandomUUID() {
  if (window.crypto && typeof window.crypto.randomUUID === 'function') return window.safeRandomUUID();
  return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function(c) {
    var r = Math.random() * 16 | 0, v = c === 'x' ? r : (r & 3 | 8);
    return v.toString(16);
  });
}
function safeShowModal(el) {
  if (!el) return;
  if (typeof el.showModal === 'function') safeShowModal(el);
  else { el.setAttribute('open',''); el.style.display='block'; }
}
function safeCloseDialog(el) {
  if (!el) return;
  if (typeof el.close === 'function') safeCloseDialog(el);
  else { el.removeAttribute('open'); el.style.display='none'; }
}
...
$('#orderForm').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget,f=new FormData(form),id=f.get('id'),file=f.get('photo');if(!id&&(!file||!file.size))return toast('د جنس عکس واخلئ');busy(form,true);try{let photoPath=editingPhoto;if(file&&file.size){photoPath=`${profile.id}/${safeRandomUUID()}.${ext(file)}`;const {error}=await db.storage.from('order-photos').upload(photoPath,file,{upsert:false});if(error)throw error}const payload={product_name:f.get('product_name').trim(),customer_phone:f.get('customer_phone').trim(),province:f.get('province'),quantity:Number(f.get('quantity')),address:f.get('address').trim(),price:Number(f.get('price')),photo_path:photoPath};const result=id?await db.from('orders').update(payload).eq('id',id):await db.from('orders').insert(payload);if(result.error)throw result.error;safeCloseDialog($('#orderDialog'));toast(id?'آرډر اصلاح شو':'آرډر ثبت شو');loadOrders()}catch(err){toast(errorText(err))}finally{busy(form,false)}};
...
function openResetPin(uid,name){const f=$('#resetPinForm');f.reset();f.uid.value=uid;$('#resetPinUser').textContent=`د ${name} لپاره نوی PIN وټاکئ.`;safeShowModal($('#resetPinDialog'))}
$('#closeResetPin').onclick=()=>safeCloseDialog($('#resetPinDialog'));
$('#resetPinForm').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget,f=new FormData(form);busy(form,true);try{const {error}=await db.rpc('admin_reset_pin',{p_user_id:f.get('uid'),p_new_pin:f.get('pin')});if(error)throw error;safeCloseDialog($('#resetPinDialog'));toast('نوی PIN ثبت شو')}catch(err){toast(errorText(err))}finally{busy(form,false)}};
...
$('#installBtn').onclick=async()=>{
  ...
  safeShowModal(if(isiPhone)$('#installDialog'));
  else toast('د نصب کړکۍ لا چمتو نه ده؛ پاڼه یو ځل تازه کړئ او بیا نصب ووهئ');
};
$('#closeInstall').onclick=$('#installOk').onclick=()=>safeCloseDialog($('#installDialog'));
$('#forgotPinBtn').onclick=()=>safeShowModal($('#forgotPinDialog'));
$('#closeForgotPin').onclick=$('#forgotPinOk').onclick=()=>safeCloseDialog($('#forgotPinDialog'));
...
var bootSafetyTimer=setTimeout(function(){
  ...
  if(loading && loading.classList.contains('active')){
    show('authView');
    toast('شبکه ورو ده؛ که معلومات ښکاره نه شول، پاڼه یو ځل تازه کړئ');
  }
},12000);
Promise.resolve(boot()).then(function(){clearTimeout(bootSafetyTimer);}).catch(function(){
