...
let ordersOffset=0,hasMoreOrders=false;
const signedMemory=new Map();
...
function ext(file){return (((file&&file.name)||'photo.jpg').split('.').pop()||'jpg').toLowerCase().replace(/[^a-z0-9]/g,'')}
function dataUrlToBlob(dataUrl){const parts=dataUrl.split(','),mime=(parts[0].match(/:(.*?);/)||[])[1]||'image/jpeg',raw=atob(parts[1]),bytes=new Uint8Array(raw.length);for(let i=0;i<raw.length;i++)bytes[i]=raw.charCodeAt(i);return new Blob([bytes],{type:mime})}
function compressImage(file,maxSide=900,quality=.68){return new Promise(resolve=>{if(!file||!file.size)return resolve(file);if(file.size<180000&&/image\/jpe?g/i.test(file.type||''))return resolve(file);const img=new Image(),url=URL.createObjectURL(file);img.onload=()=>{try{let w=img.naturalWidth||img.width,h=img.naturalHeight||img.height;const scale=Math.min(1,maxSide/Math.max(w,h));w=Math.max(1,Math.round(w*scale));h=Math.max(1,Math.round(h*scale));const canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;const ctx=canvas.getContext('2d');ctx.drawImage(img,0,0,w,h);const done=blob=>{URL.revokeObjectURL(url);resolve(blob||file)};if(canvas.toBlob)canvas.toBlob(done,'image/jpeg',quality);else done(dataUrlToBlob(canvas.toDataURL('image/jpeg',quality)))}catch(_){URL.revokeObjectURL(url);resolve(file)}};img.onerror=()=>{URL.revokeObjectURL(url);resolve(file)};img.src=url})}
...
$('#registerForm').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget,f=new FormData(form),p=phone(f.get('phone')),pin=f.get('pin');if(!/^07\d{8}$/.test(p))return toast('د موبایل شمېره سمه ولیکئ');busy(form,true);try{const {data,error}=await db.auth.signUp({email:emailFor(p),password:passwordFor(p,pin),options:{data:{full_name:f.get('name').trim(),phone:p}}});if(error)throw error;if(!data.session)throw new Error('د Email confirmation بندول پکار دي');session=data.session;const uid=data.user.id,rawPhoto=f.get('profilePhoto'),photo=await compressImage(rawPhoto,600,.7),photoPath=`${uid}/profile.jpg`;const up=await db.storage.from('identity-docs').upload(photoPath,photo,{upsert:true,contentType:'image/jpeg',cacheControl:'604800'});if(up.error)throw up.error;const {error:rpcError}=await db.rpc('set_my_identity_paths',{p_tazkira_path:null,p_selfie_path:photoPath});if(rpcError)throw rpcError;profile=await getProfile();route()}catch(err){toast(errorText(err))}finally{busy(form,false)}};
...
$('#logoutBtn').onclick=$('#pendingLogout').onclick=async()=>{await db.auth.signOut();session=null;profile=null;currentOrders=[];signedMemory.clear();localStorage.removeItem('ifcSignedUrls');$('#bottomNav').classList.add('hidden');$('#logoutBtn').classList.add('hidden');show('authView')};
...
function serviceNotice(message){const box=$('#serviceNotice');if(box){box.textContent=message;box.classList.remove('hidden')}show('authView')}
async function boot(){try{const {data,error}=await db.auth.getSession();if(error)throw error;session=data.session;if(session){try{profile=await getProfile();route()}catch(err){if(isQuotaError(err))serviceNotice('د وړیا سرور میاشتنی حد ختم شوی. حسابونه خوندي دي؛ د حد له نوي کېدو وروسته بیا هڅه وکړئ.');else serviceNotice('له سرور سره اړیکه ونه نیول شوه. انټرنېټ وګورئ او بیا هڅه وکړئ.')}}else show('authView')}catch(err){serviceNotice(isQuotaError(err)?'د وړیا سرور میاشتنی حد ختم شوی. حسابونه خوندي دي.':'له سرور سره اړیکه ونه نیول شوه. انټرنېټ وګورئ او بیا هڅه وکړئ.')}}
function route(){if(!profile)return show('authView');const notice=$('#serviceNotice');if(notice)notice.classList.add('hidden');if(profile.status!=='approved'){show('pendingView');$('#logoutBtn').classList.remove('hidden');return}$('#bottomNav').classList.remove('hidden');$('#logoutBtn').classList.remove('hidden');$('#adminNav').classList.toggle('hidden',profile.role!=='admin');$('#newOrderBtn').classList.toggle('hidden',profile.role!=='agent');$('#userName').textContent=profile.full_name;$('#avatarFallback').textContent=(profile.full_name&&profile.full_name[0])||'ا';$('#avatar').classList.remove('has-photo');if(profile.selfie_path)signed('identity-docs',profile.selfie_path).then(url=>{if(url){$('#avatarImg').src=url;$('#avatar').classList.add('has-photo')}});$('#roleBadge').textContent=roleNames[profile.role];$('#ordersTitle').textContent=profile.role==='agent'?'زما وروستي آرډرونه':'ټول وروستي آرډرونه';show('homeView');loadOrders()}

async function loadOrders(reset=true){if(reset){ordersOffset=0;currentOrders=[]}let q=db.from('orders').select('*,profiles!orders_created_by_fkey(full_name,phone)').order('created_at',{ascending:false}).range(ordersOffset,ordersOffset+ORDER_PAGE_SIZE);if(profile.role==='agent')q=q.eq('created_by',profile.id);const {data,error}=await q;if(error)return toast(errorText(error));const rows=data||[];hasMoreOrders=rows.length>ORDER_PAGE_SIZE;const page=rows.slice(0,ORDER_PAGE_SIZE);currentOrders=reset?page:currentOrders.concat(page);ordersOffset=currentOrders.length;await renderOrders();if(reset)loadStats()}
async function signed(bucket,path){if(!path)return '';const key=bucket+':'+path,now=Date.now();if(signedMemory.has(key))return signedMemory.get(key);try{const saved=JSON.parse(localStorage.getItem('ifcSignedUrls')||'{}'),hit=saved[key];if(hit&&hit.url&&hit.expires>now){signedMemory.set(key,hit.url);return hit.url}const {data,error}=await db.storage.from(bucket).createSignedUrl(path,604800);if(error)throw error;const url=(data&&data.signedUrl)||'';if(url){signedMemory.set(key,url);saved[key]={url,expires:now+6*24*60*60*1000};const keys=Object.keys(saved);if(keys.length>120)keys.sort((a,b)=>saved[a].expires-saved[b].expires).slice(0,keys.length-120).forEach(k=>delete saved[k]);localStorage.setItem('ifcSignedUrls',JSON.stringify(saved))}return url}catch(_){return ''}}
async function renderOrders(){const box=$('#ordersList');box.innerHTML='';$('#ordersEmpty').style.display=currentOrders.length?'none':'block';const cards=await Promise.all(currentOrders.map(async o=>{const url=await signed('order-photos',o.photo_path);const canEdit=profile.role==='agent'&&o.created_by===profile.id;const canMove=profile.role==='dispatcher'&&o.status==='registered';const action=o.province==='کندهار'?'تسلیم شو':'ولېږل شو';const creator=Array.isArray(o.profiles)?o.profiles[0]:o.profiles;return `<article class="order-card"><img src="${url}" alt="${escapeHtml(o.product_name)}" loading="lazy" decoding="async"><div class="order-main"><div class="order-top"><h4>${escapeHtml(o.product_name)}</h4><span class="badge ${o.status}">${statusNames[o.status]}</span></div><div class="order-meta"><span>📍 ${o.province}</span><span>📦 ${number(o.quantity)}</span><span>☎ ${escapeHtml(o.customer_phone)}</span>${profile.role!=='agent'?`<span>👤 ${escapeHtml((creator&&creator.full_name)||'نوم نه دی موندل شوی')}</span>`:''}</div><div class="order-meta"><span>📅 ثبت: ${orderDateTime(o.created_at)}</span></div><div class="order-meta"><span>📌 ادرس: ${escapeHtml(o.address)}</span></div><div class="order-foot"><strong>${number(o.price)} ؋</strong><div class="order-actions">${canEdit?`<button class="small-btn" data-edit="${o.id}">اصلاح</button>`:''}${canMove?`<button class="small-btn action" data-move="${o.id}">${action}</button>`:''}</div></div></div></article>`}));box.innerHTML=cards.join('');$$('[data-edit]').forEach(b=>b.onclick=()=>openOrder(b.dataset.edit));$$('[data-move]').forEach(b=>b.onclick=()=>advance(b.dataset.move));const more=$('#moreOrdersBtn');if(more)more.classList.toggle('hidden',!hasMoreOrders)}
function businessDayStartISO(){const key=businessDayKey(),parts=key.split('-').map(Number);return new Date(Date.UTC(parts[0],parts[1]-1,parts[2],1,30,0)).toISOString()}
async function loadStats(){let q=db.from('orders').select('province,price,status,processed_at').gte('processed_at',businessDayStartISO());if(profile.role==='agent')q=q.eq('created_by',profile.id);const {data,error}=await q;if(error)return;renderStats(data||[])}
function renderStats(rows){const kandahar=rows.filter(o=>o.province==='کندهار'&&o.status==='delivered'),other=rows.filter(o=>o.province!=='کندهار'&&o.status==='sent');$('#otherProvinceCount').textContent=number(other.length);$('#otherProvinceMoney').textContent=number(other.reduce((s,o)=>s+Number(o.price),0));$('#kandaharCount').textContent=number(kandahar.length);$('#kandaharMoney').textContent=number(kandahar.reduce((s,o)=>s+Number(o.price),0))}
...
$('#orderForm').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget,f=new FormData(form),id=f.get('id'),file=f.get('photo');if(!id&&(!file||!file.size))return toast('د جنس عکس واخلئ');busy(form,true);try{let photoPath=editingPhoto;if(file&&file.size){const small=await compressImage(file,900,.68);photoPath=`${profile.id}/${crypto.randomUUID()}.jpg`;const {error}=await db.storage.from('order-photos').upload(photoPath,small,{upsert:false,contentType:'image/jpeg',cacheControl:'604800'});if(error)throw error}const payload={product_name:f.get('product_name').trim(),customer_phone:f.get('customer_phone').trim(),province:f.get('province'),quantity:Number(f.get('quantity')),address:f.get('address').trim(),price:Number(f.get('price')),photo_path:photoPath};const result=id?await db.from('orders').update(payload).eq('id',id):await db.from('orders').insert(payload);if(result.error)throw result.error;$('#orderDialog').close();toast(id?'آرډر اصلاح شو':'آرډر ثبت شو');loadOrders(true)}catch(err){toast(errorText(err))}finally{busy(form,false)}};
async function advance(id){const o=currentOrders.find(x=>x.id===id),next=o.province==='کندهار'?'delivered':'sent';const {error}=await db.rpc('advance_order',{p_order_id:id,p_new_status:next});if(error)return toast(errorText(error));toast(statusNames[next]);loadOrders(true)}
...
async function loadReport(){let q=db.from('orders').select('province,price,status,processed_at').not('processed_at','is',null).order('processed_at',{ascending:false}).limit(5000);if(profile.role==='agent')q=q.eq('created_by',profile.id);const {data,error}=await q;if(error)return toast(errorText(error));const days=new Map();for(const o of data||[]){if(!o.processed_at)continue;const key=businessDayKey(o.processed_at);if(!days.has(key))days.set(key,{key,otherCount:0,otherMoney:0,kandaharCount:0,kandaharMoney:0});const d=days.get(key);if(o.province==='کندهار'&&o.status==='delivered'){d.kandaharCount++;d.kandaharMoney+=Number(o.price)}else if(o.province!=='کندهار'&&o.status==='sent'){d.otherCount++;d.otherMoney+=Number(o.price)}}const rows=[...days.values()].sort((a,b)=>b.key.localeCompare(a.key)),today=days.get(businessDayKey())||{otherCount:0,otherMoney:0,kandaharCount:0,kandaharMoney:0};$('#reportSummary').innerHTML=`<article><strong>${number(today.otherCount)} جنس</strong><span>نن نورو ولایتونو ته</span><b>${number(today.otherMoney)} ؋</b></article><article><strong>${number(today.kandaharCount)} جنس</strong><span>نن کندهار کې</span><b>${number(today.kandaharMoney)} ؋</b></article><article><strong>${number(today.otherCount+today.kandaharCount)}</strong><span>نن ټول</span></article>`;$('#reportUsers').innerHTML=rows.map((d,i)=>`<article class="weekly-card"><div class="weekly-title"><div><small>${i===0&&d.key===businessDayKey()?'نن':'پخوانۍ ورځ'}</small><b>${businessDayLabel(d.key)} • 06:00 — بل سهار 06:00</b></div><span>${number(d.otherCount)} نور • ${number(d.kandaharCount)} کندهار</span></div><div class="weekly-numbers split-weekly"><div><small>نورو ولایتونو ته لېږل شوي</small><strong>${number(d.otherCount)} جنس</strong><b>${number(d.otherMoney)} ؋</b></div><div class="kandahar-week"><small>کندهار کې تسلیم شوي</small><strong>${number(d.kandaharCount)} جنس</strong><b>${number(d.kandaharMoney)} ؋</b></div></div></article>`).join('')||'<div class="empty">راپور نشته</div>'}
function escapeHtml(v){return String(v==null?'':v).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]))}
function isQuotaError(e){const m=(e&&e.message)||String(e);return /exceed_(cached_)?egress_quota|restricted due to|upgrade their plan|remove spend caps/i.test(m)}
function errorText(e){const m=(e&&e.message)||String(e);if(isQuotaError(e))return 'د وړیا سرور میاشتنی حد ختم شوی؛ حسابونه خوندي دي. د حد له نوي کېدو وروسته بیا هڅه وکړئ.';if(m.includes('already registered'))return 'دا موبایل شمېره مخکې ثبت شوې';if(m.includes('row-level security'))return 'اجازه نشته؛ د اډمین سره اړیکه ونیسئ';return m}
function loginErrorText(e){const m=(e&&e.message)||String(e);if(isQuotaError(e))return 'د وړیا سرور میاشتنی حد ختم شوی؛ حسابونه خوندي دي. د حد له نوي کېدو وروسته بیا هڅه وکړئ.';if(/rate limit|too many/i.test(m))return 'ډېرې هڅې وشوې؛ لږ انتظار وکړئ او بیا هڅه وکړئ';if(/email not confirmed/i.test(m))return 'حساب لا تایید شوی نه دی: '+m;return 'د ننوتلو اصلي خطا: '+m}
